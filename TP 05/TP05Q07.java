...
class CLista{
    ...
    public boolean pesquisa(String nome){
        ...
        for(int i=0;i<quantidade;i++,tmp=tmp.prox){
            if(tmp.elemento.equals(nome)){
                return true;
            }
        }
        return false;
...
class Hash{
    ...
    public int calculaReHash(String str){
        int index=0;
        for(int i=0;i<str.length();i++){
            index+=(int)(str.charAt(i));
        }
        return ((index+1)%tamTabela);
    }
    
    public void inserir(String nome){
        int pos = calculaIndice(nome);
        lista[pos].inserirFim(nome);
    }
